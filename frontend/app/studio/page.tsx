  ...
  return (
    <AppShell>
      <div className="page-header">
        ...
        <p className="page-subtitle">Multi-input AI video generation pipeline</p>
      </div>

      {/* Mode Selector */}
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="card-header"><h3 className="card-title">Pipeline Mode</h3></div>
      ...
      <div className="grid-2">
        {/* Left: Input Slots */}
        <div className="card">
          <div className="card-header"><h3 className="card-title">Input Slots</h3></div>

          {showPersonSlot && (
            <SlotUpload label="Person / Subject" slotKey="person_image" value={slots.person_image}
          ...
          {showProductSlot && (
            <SlotUpload label="Product" slotKey="product_image" value={slots.product_image}
          ...
          {showMotionSlot && (
            <SlotUpload label="Motion Reference (video)" slotKey="motion_reference" value={slots.motion_reference}
          ...
          {showStyleSlot && (
            <SlotUpload label="Style Reference" slotKey="style_reference" value={slots.style_reference}
          ...
          {showAudioSlot && (
            <SlotUpload label="Audio Reference" slotKey="audio_reference" value={slots.audio_reference}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {(selectedMode === 'template_scene' || selectedMode === 'product_promo') && templates.length > 0 && (
            <div className="card">
              <div className="card-header"><h3 className="card-title">Scene Template</h3></div>
          ...
          {/* Prompt & Style */}
          <div className="card">
            <div className="card-header"><h3 className="card-title">Prompt & Style</h3></div>
            <div className="form-group">
              <label className="form-label">Prompt (additional instructions)</label>
              <textarea className="form-input" rows={3} value={prompt} onChange={e => setPrompt(e.target.value)}
                placeholder="e.g. excited unboxing, showing product to camera, smiling" />
            </div>
            <div className="form-group">
              <label className="form-label">Style</label>
              <input className="form-input" value={style} onChange={e => setStyle(e.target.value)}
                placeholder="e.g. UGC casual, warm lighting, cinematic" />
            </div>
          </div>
          ...
          <div className="card">
            <div className="card-header">
              <h3 className="card-title">Batch Mode</h3>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                <input type="checkbox" checked={batchMode} onChange={e => setBatchMode(e.target.checked)} />
                <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Enable</span>
              </label>
            </div>
            {batchMode && (
              <>
                <div className="form-group">
                  <label className="form-label">Variant Slot</label>
                  <select className="form-input" value={variantSlot} onChange={e => setVariantSlot(e.target.value)}>
                    <option value="person_image">Person (different people, same product)</option>
                    <option value="product_image">Product (same person, different products)</option>
                  </select>
                </div>
                <div className="form-group">
                  <label className="form-label">Variant Image URLs (one per line)</label>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <button className="btn btn-primary" onClick={handleGenerate} disabled={loading}
              ...
              {loading ? 'Generating...' : batchMode ? `Generate ${variantImages.split('\n').filter(Boolean).length} Variants` : 'Generate Video'}
            </button>
      ...
      {result && (
        <div className="card" style={{ marginTop: 20 }}>
          ...
          {result.batch_size ? (
            <div>
              <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>Batch: {result.batch_size} variants queued</p>
              <div className="activity-list">
                {result.tasks?.map((t: any) => (
                  <div key={t.id} className="activity-item">
                    <div className="activity-dot" style={{ background: t.status === 'completed' ? 'var(--success)' : 'var(--warning)' }} />
                    <span className="activity-text">Task #{t.id} — {t.status}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                <div><span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Status</span><br /><span className={`badge ${result.status === 'completed' ? 'badge-success' : 'badge-warning'}`}>{result.status}</span></div>
                <div><span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Provider</span><br /><span style={{ fontSize: 13, color: 'var(--text-primary)' }}>{result.provider}</span></div>
                <div><span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Mode</span><br /><span style={{ fontSize: 13, color: 'var(--text-primary)' }}>{result.mode}</span></div>
              </div>
              ...
              {result.prompt && (
                <div style={{ marginTop: 12, padding: 12, background: 'var(--bg-surface)', borderRadius: 8, fontSize: 12, color: 'var(--text-muted)' }}>
                  <strong>Generated Prompt:</strong><br />{result.prompt}
                </div>
              )}
            </div>
          )}
